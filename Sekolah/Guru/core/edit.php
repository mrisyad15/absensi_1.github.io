...
if (isset($_GET['edit-kelas'])) {
	...
	$nis    = $_GET['edit-siswa'];
	$siswa = mysqli_query($con,"SELECT * FROM siswa JOIN kelas on siswa.id_kelas = kelas.id_kelas WHERE nis = '$nis'");
	...
	$nis 			= htmlspecialchars($_POST['nis']);
 	...
 	$editsekolah 	= "UPDATE siswa SET nama_siswa = '$siswa', no_tlp = '$tlpn', email = '$email', alamat = '$alamat', foto = '$foto', jenis_kelamin = '$jenkel', id_kelas = '$kelas' WHERE nis = '$nis'";
	$editsekolah2 = mysqli_query($con,$editsekolah);
 	if ($editsekolah2) {
 		echo "<script>alert('Data berhasil diedit');</script>
